...
public class CustomerReg {
	...
	public static int aadharlink(CustomerRegister cr)
	{
		int result=0;
		String query;
		PreparedStatement ps=null;
		...
		try
		{
			con=DBHelper.getConnection();
			//System.out.println("insert query");
			query="INSERT INTO AADHAR(name,CARDNO) VALUES(?,?)";
			ps=con.prepareStatement(query);
			//System.out.println(cr.getName());
			ps.setString(1,cr.getName());
			ps.setString(2, cr.getCardNo());
 		...
 		{
			System.out.println("error:  customerReg:aadharlink"+e.getMessage());
		}
		return result;
	}
}
