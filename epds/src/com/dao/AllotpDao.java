...
public class AllotpDao { 
	public static int product_insert(ShopAdd shopNo)
	{
		...
		try
		{
			...
			query="INSERT INTO PRODUCTDETAILS(PRONO) VALUES(?)";
	...
	public  static ArrayList<AllotProduct> getProducts()
	{
		...
		try
		{
			String sql="SELECT SHOPDETAILS.*, PRODUCTDETAILS.* from SHOPDETAILS inner join PRODUCTDETAILS on SHOPDETAILS.SHOPNO = PRODUCTDETAILS.PRONO";
			con=DBHelper.getConnection();
	...
	public static int product_update(AllotProduct product) {
		...
		try {
			con=DBHelper.getConnection();
			query="update PRODUCTDETAILS set RICE=?,WHEAT=?,SUGAR=? where PRONO=?";
			ps=con.prepareStatement(query);
